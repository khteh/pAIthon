...
class NLPTransformer(Model):
    ...
    def __init__(self, num_layers, embedding_dim, num_heads, fully_connected_dim, input_vocab_size, target_vocab_size, max_positional_encoding_input, max_positional_encoding_target, dropout_rate=0.1, layernorm_eps=1e-6, **kwargs):
        super(NLPTransformer, self).__init__(**kwargs)
    ...
    @tf.function
    def train_step(self, inp, tar, loss_object, optimizer, train_loss):
        """
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self._masked_loss(tar_real, predictions, loss_object)

        gradients = tape.gradient(loss, self.trainable_variables)    
        optimizer.apply_gradients(zip(gradients, self.trainable_variables))
        train_loss(loss)

    def NextWord(self, encoder_input, output):
        """
    ...
    def _masked_loss(self, real, pred, loss_object):
...
def NLPTransformerTests():
    ...
    transformer = NLPTransformer(n_layers, 
        emb_d, 
        n_heads, 
        fully_connected_dim, 
        input_vocab_size, 
