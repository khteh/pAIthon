...
class GRU_CharacterGeneration():
    ...
    def BuildTrainModel(self, epochs:int, retrain:bool = False):
        ...
        if not self._model:
            ...
            self._model.compile(optimizer=Adam(learning_rate=self._learning_rate), loss=SparseCategoricalCrossentropy(from_logits=True), metrics=[log_perplexity]) # The output values generated by the model are not normalized like a probability distribution. This is our case, since our GRULM model uses a `log_softmax` activation rather than the `softmax`.
            self._model.summary()
            plot_model(
                self._model,
