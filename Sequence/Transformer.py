...
def positional_encoding(positions, d):
    ...
    angle_rads = get_angles(numpy.arange(positions)[:, numpy.newaxis],
                            numpy.arange(d)[numpy.newaxis, :],
                            d)
...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    """
    matmul_qk = q @ k.T  # (..., seq_len_q, seq_len_k)

    # scale matmul_qk
    dk = k.shape[0]
    scaled_attention_logits = matmul_qk / (dk ** 0.5)
    tmp = tf.convert_to_tensor(scaled_attention_logits)
    print(f"scaled_attention_logits: {scaled_attention_logits.shape}")
    # add the mask to the scaled tensor.
    if mask is not None: # Don't replace this None
        scaled_attention_logits += (1 - mask[0]) * -1e9

    # softmax is normalized on the last axis (seq_len_k) so that the scores
    # add up to 1.
    #print(tf.keras.activations.softmax(x + (1 - create_padding_mask(x)) * -1.0e9))
    #print(f"logits: {type(scaled_attention_logits)}")
    attention_weights = tf.keras.activations.softmax(tf.convert_to_tensor(scaled_attention_logits)) # (..., seq_len_q, seq_len_k)
...
class Encoder(Layer):
    ...
    def call(self, x, training, mask):
        ...
        x *= tf.cast(self.embedding_dim, dtype=tf.float32) ** 0.5
        ...
        print(f"encoder output: {x.shape}") # (2, 3, 4)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out1, attn_weights_block1 = self.mha1(x, x, x, attention_mask=look_ahead_mask, training=training, return_attention_scores=True)  # (batch_size, target_seq_len, embedding_dim)
